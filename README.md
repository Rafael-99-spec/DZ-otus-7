...
```
Как видим по таймеру запуска службы (29s ago при первом просмотре и 795ms ago при втором просмотре)команда указанная в findword.service указывает все строки c ключевым словом systemd, и благодаря созданному нами findword.timer юниту поиск ключевого слова осуществляется раз в 30 секунд.
