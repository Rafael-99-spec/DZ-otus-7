...
```
Далее создадим две копии оригинального конфигурационного файла httpd, и зададим в обоих файлах разные номера портов 80 и 8080 соответственно(```Listen 80``` и ```Listen 8080``` соответственно). А также укажем параметр pid-файла для второго httpd - ```PidFile /var/run/httpd-two.pid```
```
